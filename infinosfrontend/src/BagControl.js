...
const getChartData = (history, label, color) => {
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
    <div style={{
      ...
      padding: '8px 16px',
      borderRadius: '8px',
      backgroundColor: device.status ? '#D1FAE5' : '#FEE2E2',
      color: device.status ? '#065F46' : '#991B1B',
      fontWeight: '600',
      fontSize: '14px'
    }}>
      <span style={{
        ...
        height: '8px',
        borderRadius: '50%',
        backgroundColor: device.status ? '#10B981' : '#EF4444',
        animation: device.status ? 'pulse 2s infinite' : 'none'
      }}></span>
      {device.status ? '🟢 ESP32 Connected' : '🔴 ESP32 Disconnected'}
    </div>
